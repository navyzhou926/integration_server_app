/***************************
file name : md5.c
func       : deal with net comunication
***************************/
...
unsigned char md5_code[33];
unsigned char md5_digest[32];
//********************************************* //
...
unsigned char*  MD5String (char *string,unsigned int length)
{
	...
	for (i = 0; i < 16; i++)
	 {
		sprintf((char*)&(md5_code[2*i]),"%02x",(unsigned char)md5_digest[i]);
		sprintf((char*)&(md5_code[2*i+1]),"%02x",(unsigned char)(md5_digest[i]<<4));
	}
...
void  MD5String2 (char *string,char* md5_string,unsigned int length)
{
	MD5_CTX context;
	int i;
    unsigned char md5_dig[32];
    unsigned char bak[40];
    
	MD5Init (&context);
	MD5Update (&context, (unsigned char*)string,length);
	MD5Final (md5_dig, &context);

	for (i = 0; i < 16; i++)
	 {
		sprintf((char*)&(bak[2*i]),"%02x",(unsigned char)md5_dig[i]);
		sprintf((char*)&(bak[2*i+1]),"%02x",(unsigned char)(md5_dig[i]<<4));
	}

    bak[32]='\0';
    	#ifdef DEBUG_DATA
	printf("*****************************************************\n");
	printf("user password_in_MD5String2: %s\n",bak);
	printf("*****************************************************\n");
	#endif
    memcpy(md5_string, bak, 32);
...
void  MD5String22 (char *string,char* md5_string,unsigned int length)
{
    ...
    MD5String2 (string, bak1, length);
    MD5String2 (bak1, bak2, 32);

    memcpy(md5_string, bak2, 32);
...
//Function Name	:	MD5_memcpy 
//Function Use	:	
//Notes:	Encodes input (UINT4) into output (unsigned char). Assumes len is
//  a multiple of 4. 
//********************************************* //
